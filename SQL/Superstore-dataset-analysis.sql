...
select
category,Product_Name,how_many_each, round(lost,2),count(Product_Name) over() how_many_of_all, sum(lost) over () how_much_of_all
from
(
...
select
year(order_date) ,datepart(QUARTER , order_date) as q_o_d ,type_of_client,  COUNT(DISTINCT customer_id)
from
	(
	select customer_id , datepart(QUARTER , first_order_date) as q_f_o , order_date,sales,
		case 
