...
Kluczowe analizy
KPI Overview — sprzedaż / zysk / marża / liczba klientów ogółem i w czasie
Sprzedaż wg kategorii — udział % i ranking
Top 10 produktów wg zysku + Bottom 10 (produkty ze stratą)
Analiza regionalna z marżą — które regiony dowożą rentowność
Trend MoM/YoY — sprzedaż i marża w czasie (Time Intelligence DAX)
LTV per segment — Consumer / Corporate / Home Office
Discount vs marża — wpływ rabatów na zysk (próg strat)
Cohort nowi vs powracający klienci — retention w czasie
Czas dostawy — różnica Order Date vs Ship Date wg klasy wysyłki
Top 20 klientów — ich udział w zysku firmy
Kluczowe wnioski (Key Findings)
Tables: −$18k zysku przy $207k sprzedaży — strukturalna strata, nie sezonowa. Decyzja: zamknąć kategorię.
Furniture jako całość: tylko 2,4% marży — Tables i Bookcases drenują plus reszty (Chairs, Furnishings).
Technology = 17% marży, Copiers 37% — najwyższa rentowność, kierunek inwestycji.
Cubify CubeX 3D Printer: −$8 880 — pojedynczy SKU generuje największą stratę produktową.
Top 20 klientów = 21% zysku firmy (przy 22,6% marży) — retention to największa dźwignia ROI; uzasadnia program lojalnościowy.
...
Excel
Otwórz Excel/Porper_project.xlsx w Excelu (M365 — wymaga dynamicznych tablic dla XLOOKUP/FILTER).
Power Query: Dane → Odśwież wszystko.
Power BI
Otwórz Power BI/sales_superstore.pbix w Power BI Desktop.
W razie potrzeby zaktualizuj ścieżkę do pliku CSV (Transform data → Data source settings).
Nawigacja: 3 strony — Overview → Produkty → Klienci i geografia (przyciski w lewym górnym rogu).
